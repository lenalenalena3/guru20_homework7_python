...
def test_read_pdf_zip():  # чтение и проверка содержимого файла pdf из архива
    with zipfile.ZipFile(PATH_NAME_ZIP) as zip_archive:  # открываем архив
        with zip_archive.open(NAME_FILE_PDF) as pdf_file:  # открываем файл в архиве
            ...
            number_pages = pdf_reader.get_num_pages()
            for page in pdf_reader.pages:
                print(page.extract_text())
            # проверяем количество строк всего,
...
def test_read_xlsx_zip():  # чтение и проверка содержимого файла xlsx из архива
    with zipfile.ZipFile(PATH_NAME_ZIP) as zip_archive:  # открываем архив
        with zip_archive.open(NAME_FILE_XLSX) as xlsx_file:  # открываем файл в архиве
            ...
            # проверяем содержимое строк для строки заголовков, для первой строки после заголовков и для последней строки
            for sheet_name in sheet_names:
                # проверяем содержимое строк для строки заголовков, для первой строки после заголовков и для последней строки
                if sheet_name == 'Sheet1':  # для первого листа
                    sheet = workbook[sheet_name]
                ...
                # проверяем содержимое строк для первой и последней строк
                if sheet_name == 'MyLinks':  # для второго листа
                    sheet = workbook[sheet_name]
                    for row_index, row_sheet in enumerate(sheet.iter_rows(values_only=True), start=1):
                        if row_index == 2:
...
def test_read_csv_zip():  # чтение и проверка содержимого файла csv из архива
    ...
    with zipfile.ZipFile(PATH_NAME_ZIP) as zip_archive:  # открываем архив
        with zip_archive.open(NAME_FILE_CSV) as csv_file:  # открываем файл в архиве
            ...
            count = csv_reader.__len__()
            for i in range(count):
                row[i] = csv_reader[i]
                print(row[i])
            # проверяем количество строк всего, корректность заголовков,
            # содержимое первой строки после заголовков и содержимое последней строки
            assert count == 20
            ...
            assert row[count - 1] == ['Mr. Jaiden Johns', '659-533-8311', 'clarkin@stroman.com',
